        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
          {sites.map((site, index) => (
            <div key={index} className="relative group w-full max-w-xs mx-auto">
              <Link href={site.url}>
                <div className={`bg-indigo-700 text-white text-center py-10 px-4 cursor-pointer hover:bg-indigo-800 transition duration-300 ${openSite === index ? "rounded-t-md" : "rounded-md"}`}>
                  ...
                  <div className={`absolute inset-0 bg-gray-800 bg-opacity-75 opacity-0 md:group-hover:opacity-100 flex items-center justify-center transition duration-300 ${openSite === index ? "rounded-t-md" : "rounded-md"}`}>
              ...
              {openSite === index && (
                <div className="z-20 h-32 absolute w-full bg-gray-800 text-white p-4 rounded-b-md transition duration-300">
                  <h4 className="text-lg font-semibold">{site.name}</h4>
                  <p className="text-sm">{site.url}</p>
                  <p className="text-xs mt-2">{site.description}</p>
                </div>
